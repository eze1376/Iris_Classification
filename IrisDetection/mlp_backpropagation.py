...
def initialization(isTest, SepalLengthCm, SepalWidthCm, PetalLengthCm, PetalWidthCm):
# Load datase
    global test_y, test_X, train_X, train_y
    with open('IrisDetection\\Iris.csv') as csvfile:
    ...
    if isTest:
        test_X = [[float(SepalLengthCm), float(SepalWidthCm), float(PetalLengthCm), float(PetalWidthCm)]]
        test_y = [1]
    else:
...
def predictor(Alpha, Epoch, Hidden_Neuron):
    ...
    epoch = Epoch
    # epoch = 500
    neuron = [4, int(Hidden_Neuron), 3]  # number of neuron each layer
...
def main(isTest, Alpha = .002, Epoch = 500, Hidden_Neuron = 5, SepalLengthCm = None, SepalWidthCm = None, PetalLengthCm = None, PetalWidthCm = None):
    result_values.clear()
    initialization(isTest, SepalLengthCm, SepalWidthCm, PetalLengthCm, PetalWidthCm)
    return predictor(Alpha, Epoch, Hidden_Neuron)
